...
export const getCrits = async (
  guildID: string,
  member: Member
): Promise<Crits> => {
  const res = await db.getUserCrits(guildID, member.id);
  if (res[0]) return critsFromRow(res[0]);
  else return { userID: member.id, Crit20: 0, Crit1: 0 };
};
